...
void BombermanBomb::Explode(MI0283QT9 * LCD, unsigned char explotionTime){
	...
	for (int blast = 0; blast < this->Player->Blastpower; blast++)
	{
		...
		if( !(directions & 0x01) && this->Y+blast+1 < _game->MaxY && _game->Grid[this->X][this->Y+blast+1]  != _game->Wall){
			if(_game->Grid[this->X][this->Y+blast+1]  == _game->Rock || (explotionTime > 0 && _game->Grid[this->X][this->Y+blast+1]  < _game->Explotion)){
				directions |= 0x01;
			}

			if(explotionTime == 0 || !(directions & 0x01)){
				if(_game->Grid[this->X][this->Y+blast+1]  > _game->Rock)
					_game->Grid[this->X][this->Y+blast+1] &= ~_game->Rock;
				else
		...
		if(!(directions & 0x04) && this->X+blast+1 < _game->MaxX && _game->Grid[this->X+blast+1][this->Y] != _game->Wall){
			if(_game->Grid[this->X+blast+1][this->Y]  == _game->Rock || (explotionTime > 0 && _game->Grid[this->X+blast+1][this->Y]  < _game->Explotion)){
				directions |= 0x04;
			}

			if(explotionTime == 0 || !(directions & 0x04)){
				if(_game->Grid[this->X+blast+1][this->Y]  > _game->Rock)
					_game->Grid[this->X+blast+1][this->Y] &= ~_game->Rock;
				else
